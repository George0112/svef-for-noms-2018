/*
 * Copyright (c) from 2000 to 2011
 * 
 * EstiNet Technologies Inc.
 * All Rights Reserved.
 * 
 * This source code file is part of the EstiNet network simulator and emulator.
 * It is an intellectual property of EstiNet Technologies Inc..
 * Without written permissions obtained from EstiNet Technologies Inc., it
 * is prohibited by law to disclose, transmit, post, or distribute this 
 * source code file to any one or any place on the Internet that is 
 * unauthorized by EstiNet Technologies Inc. to receive this source code file.
 * 
 * 3/31/2011
 */
...
int bw = 100, qlen = 50, delay = 0,
	port = 3000, repeat, pkt_no, tcp, dex,
	alarmflag, countflag,
...
{
	int tos, trsfd, len, k, trtotalsize = 0;
        unsigned long int frameno;
	double inval, trstime;
	char buffer[20000], buf[100];
	FILE *trpt;
	struct hostent *trhp;
	char _trinput[100];
	...
	while (fscanf(trpt, "%d %lf %d %ld", &len, &inval, &tos, &frameno) != EOF) {
		id++;
		if (SHOW || BUG)
		...
		if (SHOW || BUG)
			printf("%10.6lf\n", trstime);
                ...
                sinfo.id=id;
                sinfo.time=trstime;
                sinfo.frameno=frameno;
                sinfo.tos = tos;
                memcpy(buffer, &sinfo, sizeof(struct sent_info));      
		//sprintf(buffer, "%d+%lf+", id, trstime);
      
                if(setsockopt(trsfd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos))) {
		...
		tos=0;
		if(setsockopt(trsfd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos))) {
                        printf("2:TOS setting fails, tos = %d \n", tos);
			errorExit();
                }
	}
        printf("%d packets are generated\n", id); 
	fclose(trpt);
